...
# THIS SCRIPT WLL CREATE A VCF, HF-VCF, and BAM FILE per sample
# THE BAM FILE WILL BE MOVED TO THE BAM FOLDER IN THE PROJECT DIRECTORY
...
/opt/edico/bin/dragen -f -r $REF_HASH_DIR --fastq-list $FASTQ_LIST --vc-sample-name $SM_TAG --enable-variant-caller true --vc-reference $REF_GENOME --output-directory $OUT_DIR'/DRAGEN_OUTPUT/' --intermediate-results-dir /staging/tmp --generate-md-tags true --preserve-bqsr-tags true --enable-duplicate-marking false --enable-map-align-output true --enable-bam-indexing true --output-file-prefix $SM_TAG --enable-vqsr true --vc-max-alternate-alleles 3 --vqsr-config-file /opt/edico/config/dragen-VQSR.cfg --vqsr-annotation INDEL,DP,QD,FS,MQRankSum,ReadPosRankSum --vqsr-annotation SNP,QD,ReadPosRankSum,FS,DP  --vqsr-resource SNP,15.0,$HAPMAP --vqsr-resource SNP,12.0,$OMNI_VCF --vqsr-resource SNP,10.0,$ONEKG_HIGH_CONF --vqsr-resource SNP,2.0,$DBSNP_138 --vqsr-resource INDEL,12.0,$ONEKG_GOLD_STD --vqsr-resource INDEL,2.0,$DBSNP_138 --vqsr-tranche 100.0 --vqsr-tranche 99.9 --vqsr-tranche 99.8 --vqsr-tranche 99.7 --vqsr-tranche 99.6 --vqsr-tranche 99.5 --vqsr-tranche 99.4 --vqsr-tranche 99.3 --vqsr-tranche 99.2 --vqsr-tranche 99.1 --vqsr-tranche 99.0 --vqsr-tranche 98.0 --vqsr-tranche 97.0 --vqsr-tranche 96.0 --vqsr-tranche 95.0 --vqsr-tranche 90.0 --vqsr-filter-level SNP,99.5 --vqsr-filter-level INDEL,99.0 --dbsnp $DBSNP_138
# dragen -f -r $REF_HASH_DIR --fastq-list $FASTQ_LIST --vc-sample-name $SM_TAG --enable-variant-caller true --vc-reference $REF_GENOME --output-directory $OUT_DIR'/DRAGEN_OUTPUT/' --intermediate-results-dir /staging/tmp --generate-md-tags true --preserve-bqsr-tags true --enable-duplicate-marking false --enable-map-align-output true --enable-bam-indexing true --combine-samples-by-name=true --output-file-prefix $SM_TAG --dbsnp $DBSNP_138 --RGLB $SM_TAG --RGPL ILLUMINA --RGDT '2016-01-16T19:00:00-0500' --RGSM $SM_TAG --RGDS HiSeq-X

END_FROM_FASTQ=`date '+%s'`

mv $OUT_DIR'/DRAGEN_OUTPUT/'$SM_TAG'.bam*' $OUT_DIR'/BAM/'$SM_TAG'.bam'
echo 'FROM_FASTQ,A.00,'$START_FROM_FASTQ','$END_FROM_FASTQ >> $OUT_DIR/REPORTS/run_times.csv
