...
ls $CORE_PATH/$PROJECT/SNV/MULTI/WHOLE_GENOME/PASS/*.vcf \
| awk '{split($1,SMtag,"/");print "grep -v \x22^#\x22","'$CORE_PATH'""/""'$PROJECT'""/SNV/MULTI/WHOLE_GENOME/PASS/"SMtag[11],\
"| awk \x27{t++NR} {s+=($3~\x22rs\x22)} END {print \x22"SMtag[11]"\x22,(s/t*100)}\x27"}' \
| bash \
| sed 's/.WHOLE.GENOME.SNV.PASS.ALL.vcf//g' \
| sed 's/ /\t/g' \
...
ls $CORE_PATH/$PROJECT/SNV/MULTI/CODING/PASS/*.vcf \
| awk '{split($1,SMtag,"/");print "grep -v \x22^#\x22","'$CORE_PATH'""/""'$PROJECT'""/SNV/MULTI/CODING/PASS/"SMtag[11],\
"| awk \x27{t++NR} {s+=($3~\x22rs\x22)} END {print \x22"SMtag[11]"\x22,(s/t*100)}\x27"}' \
| bash \
| sed 's/.CODING.SNV.PASS.ALL.vcf//g' \
| sed 's/ /\t/g' \
...
ls $CORE_PATH/$PROJECT/INDEL/MULTI/WHOLE_GENOME/PASS/*.vcf \
| awk '{split($1,SMtag,"/");print "grep -v \x22^#\x22","'$CORE_PATH'""/""'$PROJECT'""/INDEL/MULTI/WHOLE_GENOME/PASS/"SMtag[11],\
"| awk \x27{count++NR} \
...
| bash \
| sed 's/.WHOLE.GENOME.INDEL.PASS.ALL.vcf//g' \
| sed 's/ /\t/g' \
...
ls $CORE_PATH/$PROJECT/INDEL/MULTI/CODING/PASS/*.vcf \
| awk '{split($1,SMtag,"/");print "grep -v \x22^#\x22","'$CORE_PATH'""/""'$PROJECT'""/INDEL/MULTI/CODING/PASS/"SMtag[11],\
"| awk \x27{count++NR} \
{count_dbsnp+=($3~\x22rs\x22)} \
{big_i_nm+=(length($5)-length($4))>15&&$5!~\x22,\x22} \
{big_i_nm_dbsnp+=(length($5)-length($4))>15&&$5!~\x22,\x22&&$3~\x22rs\x22} \
...
(bp_two_i_nm/bp_three_i_nm),(bp_two_d_nm/bp_three_d_nm)}\x27"}' \
| bash \
| sed 's/.CODING.INDEL.PASS.ALL.vcf//g' \
| sed 's/ /\t/g' \
