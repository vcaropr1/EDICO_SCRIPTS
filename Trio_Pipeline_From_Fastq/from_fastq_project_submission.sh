...
FASTQ_DIR=$2 # WHERE THE FASTQ FILES ARE LOCATED.  ALL FILES MUST BE IN ONE DIRECTORY AND CONTAIN THE RGPU_1/2 IN THE NAMES
OUT_DIR=$3 # PROJECT MAIN DIRECTORY

SCRIPT_DIR='/isilon/sequencing/VITO/NEW_GIT_REPO/EDICO_SCRIPTS/Trio_Pipeline_From_Fastq'
...
$SCRIPT_DIR'/fastq_list_maker.sh' $SAMPLE_SHEET $FASTQ_DIR $OUT_DIR
...
do
FASTQ_LIST=`echo $OUT_DIR'/'$SAMPLE'_fastq_list.csv'`

VCF_JOB_ID=$(sbatch --gres=dragen_board:1 -o $OUT_DIR/LOGS/$SAMPLE'_FROM_FASTQ_VCF.log' $SCRIPT_DIR/from_fastq_vcf.sh $SAMPLE $FASTQ_LIST $OUT_DIR $REF_HASH_DIR $REF_GENOME $DBSNP_138 $HAPMAP $OMNI_VCF $ONEKG_HIGH_CONF $ONEKG_GOLD_STD | awk '{print $4}')

GVCF_JOB_ID=$(sbatch --gres=dragen_board:1 -o $OUT_DIR/LOGS/$SAMPLE'_FROM_FASTQ_GVCF.log' $SCRIPT_DIR/from_fastq_gvcf.sh $SAMPLE $FASTQ_LIST $OUT_DIR $REF_HASH_DIR $REF_GENOME $DBSNP_138 $HAPMAP $OMNI_VCF $ONEKG_HIGH_CONF $ONEKG_GOLD_STD | awk '{print $4}')
